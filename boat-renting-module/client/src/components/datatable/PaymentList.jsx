...
const PaymentList = () => {
  ...
  const handleExportToPDF = () => {
    ...
    const columns = paymentColumn.map((column) => column.headerName);
    ...
    const rows = payment.map((item) => {
      ...
      columns.forEach((column) => {
        ...
        if (column === "Id") {
          ...
          value = item.amount || ""; // Access the nested property directly
        } else if (column === "Payment Status") {
          value = item.paymentStatus || ""; // Access the nested property directly
        } else if (column === "Internal Payment Ref") {
        
          value = item.iniPaymentRef || ""; // Access the nested property directly
        } else if (column === "External Payment Ref") {
          value = item.extPaymentRef || ""; // Access the nested property directly
        } else if (column === "Booking Id") {
          value = item.bookingId || ""; // Access the nested property directly
        } else if (column === "Payment Date") {
          const readableDate = new Date(item.created_at).toLocaleDateString();
          value = readableDate || ""; // Access the nested property directly
        } else {
          const field = paymentColumn.find(
    ...
    // Add a title
    doc.text("List Of All Payments", 105, 10, { align: "center" });
    ...
    // Save the PDF
    doc.save("payment_report_list.pdf");
  };
